...
namespace lve 
{
    ...
    void LveSwapChain::createSwapChain() 
    {
      ...
      // only specified a minimum number of images in the swap chain, so the implementation is
    ...
    {
        //handles synchronisation with the display
        //using a mailbox synchronisation will not cause the gpu to idle when the swap chain is swapping the frame.
        //what happens is the gpu will constantly generate a new frame until it is swapped which reduces input lag 
        //but increases power consumption therefore FIFO synchronisation may be more useful in mobile devices.
